<template>
  <div>
    <h2 id="page-heading" data-cy="MenuHeading">
      <span v-text="$t('gatewayApp.authMenu.home.title')" id="menu-heading">Menus</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('gatewayApp.authMenu.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'MenuCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-menu">
            ...
            <span v-text="$t('gatewayApp.authMenu.home.createLabel')"> Create a new Menu </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && menus && menus.length === 0">
      <span v-text="$t('gatewayApp.authMenu.home.notFound')">No menus found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('gatewayApp.authMenu.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('gatewayApp.authMenu.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('link')">
              <span v-text="$t('gatewayApp.authMenu.link')">Link</span>
            ...
            <th scope="row" v-on:click="changeOrder('method')">
              <span v-text="$t('gatewayApp.authMenu.method')">Method</span>
            ...
            <th scope="row" v-on:click="changeOrder('authKey')">
              <span v-text="$t('gatewayApp.authMenu.authKey')">Auth Key</span>
            ...
            <th scope="row" v-on:click="changeOrder('showed')">
              <span v-text="$t('gatewayApp.authMenu.showed')">Showed</span>
            ...
            <th scope="row" v-on:click="changeOrder('parent')">
              <span v-text="$t('gatewayApp.authMenu.parent')">Parent</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="gatewayApp.authMenu.delete.question" data-cy="menuDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="au-delete-menu-heading" v-text="$t('gatewayApp.authMenu.delete.question', { id: removeId })">
