...
import cc.microthink.auth.config.EmbeddedKafka;
...
class AuthKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/auth-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
