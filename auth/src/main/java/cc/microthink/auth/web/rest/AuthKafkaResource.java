...
import cc.microthink.auth.config.KafkaSseConsumer;
import cc.microthink.auth.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/auth-kafka")
public class AuthKafkaResource {

    private final Logger log = LoggerFactory.getLogger(AuthKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public AuthKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
