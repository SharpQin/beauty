...
import cc.microthink.shipping.config.EmbeddedKafka;
import cc.microthink.shipping.config.KafkaSseConsumer;
import cc.microthink.shipping.config.KafkaSseProducer;
...
class ShippingKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/shipping-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/shipping-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/shipping-kafka/unregister"));
                return;
            }
        }
