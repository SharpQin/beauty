...
import cc.microthink.payment.config.EmbeddedKafka;
import cc.microthink.payment.config.KafkaSseConsumer;
import cc.microthink.payment.config.KafkaSseProducer;
...
class PaymentKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/payment-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/payment-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/payment-kafka/unregister"));
                return;
            }
        }
